...
export default function Navbar() {
  ...
  return (
    <nav style={{ backgroundColor: '#FDFAF5', borderBottom: '1px solid #EDE0CC' }} className="sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <Link to="/" className="font-sans text-2xl font-light text-shaanti-800 tracking-widest uppercase">
          Shaanti
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              ...
              <button onClick={handleLogout} className="font-body text-sm text-shaanti-400 hover:text-shaanti-700 flex items-center gap-1.5 transition-colors">
        ...
        {/* Burger mobile */}
        <button className="md:hidden p-2 text-shaanti-600" onClick={() => setOpen(o => !o)}>
      ...
      {/* Menu mobile */}
      {open && (
        <div style={{ backgroundColor: '#F7F0E6', borderTop: '1px solid #EDE0CC' }} className="md:hidden px-6 py-4 space-y-4">
          {NAV_LINKS.map(l => (
          ...
          <div className="pt-2 border-t border-shaanti-200 flex gap-3">
            {user ? (
              <>
                <Link to={user.role === 'pro' ? '/pro-espace' : '/espace'} onClick={() => setOpen(false)} className="btn-secondary text-xs py-2">Mon espace</Link>
                <button onClick={handleLogout} className="btn-secondary text-xs py-2">Déconnexion</button>
              </>
            ) : (
              <>
